...
fun OnboardingScreen(
	...
	Scaffold(
		modifier = modifier,
		topBar = { TopAppBar(title = { Text("Set up ClipLex") }) },
	) { contentPadding ->
		Column(
			modifier = Modifier
				.fillMaxSize()
			...
			LazyColumn(
				...
				item(key = "learning_language") {
					OnboardingSection(
						...
						selectedLearningLanguage?.let { language ->
							val statusText = when (val status = speechLanguageStatus) {
								SpeechLanguageStatus.Ready -> "✓ Ready"
								SpeechLanguageStatus.DownloadRequired -> "↓ Download ${language.displayName} speech support"
								is SpeechLanguageStatus.Downloading -> status.progress?.let { "Downloading… $it%" }
									?: "${language.displayName} speech support will download when available."
								SpeechLanguageStatus.AndroidUnsupported -> if (fallbackSpeechReady) "✓ Ready" else "Additional speech support required"
								is SpeechLanguageStatus.Error -> if (fallbackSpeechReady) "✓ Ready" else "Additional speech support required"
							}
							Text(
								text = statusText,
								style = MaterialTheme.typography.bodySmall,
								color = if (speechLanguageStatus is SpeechLanguageStatus.Ready) {
						...
						OnboardingSection(
							...
							Column(
								...
								SpeechQualityChoice(
									...
									description = "Best Indian-language recognition · 181 MB download",
									badge = "Strongest",
			...
			Button(
				onClick = if (speechDownloadRequired) onDownloadSpeech else onContinue,
				enabled = selectedMotherTongue != null && selectedLearningLanguage != null && !isSaving && !speechDownloading,
				modifier = Modifier
					.fillMaxWidth()
				...
				if (speechDownloadRequired) {
					Icon(Icons.Default.Download, contentDescription = null, modifier = Modifier.size(18.dp))
					Spacer(Modifier.width(8.dp))
				}
				Text(when {
					isSaving -> "Saving…"
					speechDownloadRequired -> "Download & Continue"
					speechDownloading -> "Preparing speech support…"
					else -> "Continue"
				})
			}
		}
