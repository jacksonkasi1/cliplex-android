...
Downloads use HTTPS from the official `ggerganov/whisper.cpp` Hugging Face repository, resume with HTTP Range from a private `.part` file, and are atomically renamed only after size and SHA-256 validation. Duplicate taps reuse the active download job. Tiny is loaded by default and the native context is reused across captures.
